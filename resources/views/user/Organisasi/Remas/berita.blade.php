...
<div class="container py-2">
    @if ($berita_remas->hasPages())
    <ul class="pagination" role="navigation">
        {{-- Previous Page Link --}}
        @if ($berita_remas->onFirstPage())
        <li class="page-item disabled" aria-disabled="true" aria-label="@lang('pagination.previous')">
            <span class="page-link" aria-hidden="true">&lsaquo;</span>
        </li>
        @else
        <li class="page-item">
            <a class="page-link" href="{{ $berita_remas->previousPageUrl() }}" rel="prev" aria-label="@lang('pagination.previous')">&lsaquo;</a>
        </li>
        @endif

        <?php
        $start = $berita_remas->currentPage() - 2; // show 3 pagination links before current
        $end = $berita_remas->currentPage() + 2; // show 3 pagination links after current
        if ($start < 1) {
            $start = 1; // reset start to 1
            $end += 1;
        }
        if ($end >= $berita_remas->lastPage()) $end = $berita_remas->lastPage(); // reset end to last page
        ?>

        @if($start > 1)
        <li class="page-item">
            <a class="page-link" href="{{ $berita_remas->url(1) }}">{{1}}</a>
        </li>
        @if($berita_remas->currentPage() != 4)
        {{-- "Three Dots" Separator --}}
        <li class="page-item disabled" aria-disabled="true"><span class="page-link">...</span></li>
        @endif
        @endif
        @for ($i = $start; $i <= $end; $i++) <li class="page-item {{ ($berita_remas->currentPage() == $i) ? ' active' : '' }}">
            <a class="page-link" href="{{ $berita_remas->url($i) }}">{{$i}}</a>
            </li>
            @endfor
            @if($end < $berita_remas->lastPage())
                @if($berita_remas->currentPage() + 3 != $berita_remas->lastPage())
                {{-- "Three Dots" Separator --}}
                <li class="page-item disabled" aria-disabled="true"><span class="page-link">...</span></li>
                @endif
                <li class="page-item">
                    <a class="page-link" href="{{ $berita_remas->url($berita_remas->lastPage()) }}">{{$berita_remas->lastPage()}}</a>
                </li>
                @endif

                {{-- Next Page Link --}}
                @if ($berita_remas->hasMorePages())
                <li class="page-item">
                    <a class="page-link" href="{{ $berita_remas->nextPageUrl() }}" rel="next" aria-label="@lang('pagination.next')">&rsaquo;</a>
                </li>
                @else
