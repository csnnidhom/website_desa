...
<div class="container-fluid py-3">
    <div class="container py-3">
        <nav class="breadcrumbs mb-5 wow zoomIn" data-wow-delay="1.7s">
...
<div class="container py-2">
    @if ($berita_kartar->hasPages())
    <ul class="pagination" role="navigation">
        {{-- Previous Page Link --}}
        @if ($berita_kartar->onFirstPage())
        <li class="page-item disabled" aria-disabled="true" aria-label="@lang('pagination.previous')">
            <span class="page-link" aria-hidden="true">&lsaquo;</span>
        </li>
        @else
        <li class="page-item">
            <a class="page-link" href="{{ $berita_kartar->previousPageUrl() }}" rel="prev" aria-label="@lang('pagination.previous')">&lsaquo;</a>
        </li>
        @endif

        <?php
        $start = $berita_kartar->currentPage() - 2; // show 3 pagination links before current
        $end = $berita_kartar->currentPage() + 2; // show 3 pagination links after current
        if ($start < 1) {
            $start = 1; // reset start to 1
            $end += 1;
        }
        if ($end >= $berita_kartar->lastPage()) $end = $berita_kartar->lastPage(); // reset end to last page
        ?>

        @if($start > 1)
        <li class="page-item">
            <a class="page-link" href="{{ $berita_kartar->url(1) }}">{{1}}</a>
        </li>
        @if($berita_kartar->currentPage() != 4)
        {{-- "Three Dots" Separator --}}
        <li class="page-item disabled" aria-disabled="true"><span class="page-link">...</span></li>
        @endif
        @endif
        @for ($i = $start; $i <= $end; $i++) <li class="page-item {{ ($berita_kartar->currentPage() == $i) ? ' active' : '' }}">
            <a class="page-link" href="{{ $berita_kartar->url($i) }}">{{$i}}</a>
            </li>
            @endfor
            @if($end < $berita_kartar->lastPage())
                @if($berita_kartar->currentPage() + 3 != $berita_kartar->lastPage())
                {{-- "Three Dots" Separator --}}
                <li class="page-item disabled" aria-disabled="true"><span class="page-link">...</span></li>
                @endif
                <li class="page-item">
                    <a class="page-link" href="{{ $berita_kartar->url($berita_kartar->lastPage()) }}">{{$berita_kartar->lastPage()}}</a>
                </li>
                @endif

                {{-- Next Page Link --}}
                @if ($berita_kartar->hasMorePages())
                <li class="page-item">
                    <a class="page-link" href="{{ $berita_kartar->nextPageUrl() }}" rel="next" aria-label="@lang('pagination.next')">&rsaquo;</a>
                </li>
                @else
